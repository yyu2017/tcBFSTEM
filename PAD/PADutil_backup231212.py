...
#     print("c_xu,c_xd",c_xu,c_xd)
#     print("c_yl,c_yr",c_yl,c_yr)
    ...
    for i in range(xdim):
        for j in range(ydim):
            new_x1=(i*scale+xint)%xdim0
            new_y1=(j*scale+yint)%ydim0
            new_x2=min(new_x1+scale,xdim0)
            new_y2=min(new_y1+scale,ydim0)
            
            upshift_Sum[new_x1:new_x2,new_y1:new_y2]=upshift_Sum[new_x1:new_x2,new_y1:new_y2]
            +im[i*scale,j*scale]*tile[0:new_x2-new_x1,0:new_y2-new_y1]

    tile_full = np.tile(tile,(xdim, ydim))

    shift_im = upshift_Sum
...
def shiftfunc_backup(im,xs,ys,scale,stype='real',zeros=None):
...
def apply_shift_expand_backup(stack,xshifts,yshifts,scale,stype='real',noise=False,snr=10):
    x,y,z = np.shape(stack)
    stack_reg = np.zeros((x*scale,y*scale,z))
    norm = np.zeros((x*scale,y*scale))
...
#         pbar = tqdm(total = z,desc = "registering")
        for i in range(z):
            ...
            stack_reg[:,:,i],temp = shiftfunc_backup(upstack, xshifts[i]*scale, yshifts[i]*scale,scale,stype=stype,zeros=zeros)
            norm += temp
#             pbar.update(1)
