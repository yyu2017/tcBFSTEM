...
class PADdata(object):
    ...
    def __init__(self, filename, EMPAD, dtype='float32', voltage=300, 
        ...
        if EMPAD == 2:
            self.data = util.readPAD(filename, EMPAD = EMPAD, dtype = dtype)[:, :, :, :]
        if EMPAD == 1:
            self.data = util.readPAD(filename, EMPAD = EMPAD, dtype = dtype)[1:-2, 3:-2, :, :]
