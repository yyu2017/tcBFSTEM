...
def enhance_contrast(array, p):
    #saturate p percent of pixels in the image to spread the histogram over larger display range.
    n = int(np.floor(np.prod(np.shape(array))*p/200))
    a = np.unravel_index(np.argsort(array, axis = None), np.shape(array))
    return np.clip(array, array[a][n], array[a][-n-1])
...
def save_matShow(array, filename, cmap='bwr', v=False, vmin=False, vmax=False, **kwargs):
    fig = plt.figure(**kwargs)
    if vmin==False and vmax==False and v==False:
        v = np.max(abs(array), axis=None)
        vmin = -v
...
def fast_radial_profile(array,pixsize=1):
    ...
    r = r.astype(int)
    tbin = np.bincount(r.ravel(), crop.ravel())
    nr = np.bincount(r.ravel())
    profile_temp = tbin / nr
    profile = profile_temp[:rad]
    ## calulate k space
    k = np.arange(0,int(dim/2),1)/(dim*pixsize)
    
    return k,profile,crop